...
const createButton = (types = ["","","",""], item,controller, className) => {
    ...
    return `<a ${types[3] ? types[3] :''} data-href="${webAdminUrl}/${controller}/${types[1]}/${item.id}" href="#" ${types[2] ? 'data-get="'+webAdminUrl+'/'+controller+'/'+types[2]+'/'+item.id+'"' : ''} class="btn btn-hnn btn-hnn-${color} ${action}${className}"><span data-toggle="tooltip" title="${title}" ><i class="fas fa-${icon}"></i></span></a>`
}
// Show button update
