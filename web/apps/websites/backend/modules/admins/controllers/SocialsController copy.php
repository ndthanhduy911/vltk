...
use Backend\Modules\Admins\Forms\SocialsForm;
use Backend\Modules\Admins\Forms\SocialsLangForm;
use Backend\Modules\Admins\Forms\SearchSocialsForm;

class SocialsController  extends \BackendController {

    private $title = "Biểu tượng";

    private $cler = "socials";

    private $className = \Socials::class;
    ...
    public function indexAction(){
        ...
        $this->view->searchForm = new SearchSocialsForm();
    ...
    public function viewAction($id = 0){
        ...
        if($id){
            ...
            foreach ($languages as $key => $lang) {
                $v = ($key == 0 ? true : false);
                $itemsLang = \SocialsLang::findFirst(['pageid = :id: AND langid = :langid:','bind' => ['id' => $page->id, 'langid' => $lang->id]]);
                if($itemsLang){
                    $formLang = new SocialsLangForm($itemsLang, [$lang->id,$v]);
                    $formsLang[$lang->id] = $formLang;
                }else{
                    $formsLang[$lang->id] = new SocialsLangForm(null, [$lang->id,$v]);
                }
            }
            $title = 'Chỉnh sửa';
        }else{
    ...
    public function updateAction($id = 0){
        ...
        foreach ($languages as $key => $lang) {
            ...
            if(!$id || !$pageLang = \SocialsLang::findFirst(["pageid = :id: AND langid = :langid:",'bind' => ['id' => (int)$id,'langid' => $lang->id]])){
                $pageLang = new \SocialsLang();
            }
    ...
    private function deleteOne($item)
    {
        ...
        \Logs::saveLogs($this, 3, "Xóa tạm trang ID: {$item->id}", ['table' => 'Socials','id' => $item->id]);
    }

    private function getJsCss(){
        $this->assets->addJs(WEB_URI.'/assets/backend/js/modules/admins/socials.js');
    }
}