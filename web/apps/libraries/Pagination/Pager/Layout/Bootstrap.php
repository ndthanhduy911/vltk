 ...
 */
namespace Library\Pagination\Pager\Layout;

use Library\Pagination\Pager\Layout;
...
class Bootstrap extends Layout
{
    ...
    public function getRendered(array $options = array())
    {
        ...
        $this->addMaskReplacement('page', 'Start', true);
        $options['page_number'] = $this->pager->getFirstPage();
        $result .= $this->processPage($options);
        $this->addMaskReplacement('page', 'Back', true);
        ...
        $result .= $this->processPage($options);
        $this->addMaskReplacement('page', 'End', true);
