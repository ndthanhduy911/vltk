                                ...
                                <div class="collapse navbar-collapse" id="navbar-collapse-1">
                                    <ul class="navbar-nav ml-xl-auto">
                                        ...
                                        <li class="nav-item dropdown">
                                            <a target="<?= $this->helper->getTarget($menu->target) ?>" href="<?= Menus::getLink($menu, $slug) ?>" class="nav-link <?= $menuChild->count() ? 'dropdown-toggle' : '' ?>" <?= $menuChild->count() ? 'data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"' : '' ?>><?= Menus::getName($menu->id, $lang_id) ?></a>
                                            <?php if($menuChild->count()){ ?>
                                                <ul class="dropdown-menu">
                                                <?php foreach ($menuChild as $child) { ?>
                                                    <li><a target="<?= $this->helper->getTarget($child->target) ?>" href="<?= Menus::getLink($child, $slug) ?>" class=""><?= Menus::getName($child->id, $lang_id) ?></a></li>
                                                <?php } ?>
                                                </ul>
                                            <?php } ?>
                                        </li>
